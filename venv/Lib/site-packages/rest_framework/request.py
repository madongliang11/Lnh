...
class Request:
    ...
    def _parse(self):
        ...
        # 选择解析器
        # self.parsers：request中封装的所有解析器对象
        # self：请求对象
    ...
    def _authenticate(self):
        ...
        for authenticator in self.authenticators:
            try:
                ...
                # 1、如果authenticate()方法抛出异常，self._not_authenticated()执行
                # 2、如果authenticate()方法没有抛出异常，并且user_auth_tuple有返回值，将元组的第一个值赋值给self.user，第二个值给self.auth
                # 即user_auth_tuple如果有返回值，必须是元组：（request.user, request.auth）
                # 3、如果返回None，表示我不管，下一个认证来处理
            ...
            if user_auth_tuple is not None:
                ...
                return
        # 如果所有的认证对象返回值都是None,执行_not_authenticated()方法，self.user, self.auth会给一个默认值
        # 即self.user = AnonymousUser，self.auth = None
        self._not_authenticated()
    ...
    def _not_authenticated(self):
        ...
        if api_settings.UNAUTHENTICATED_USER:
            self.user = api_settings.UNAUTHENTICATED_USER() # 默认值AnonymousUser（匿名用户）
        else:
            self.user = None

        if api_settings.UNAUTHENTICATED_TOKEN:
            self.auth = api_settings.UNAUTHENTICATED_TOKEN() # 默认值None
        else:
            self.auth = None
